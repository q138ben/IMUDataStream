...
namespace Playground
{
     class Program
     {
        ...
        private void Capture_DataAvailable(object sender, DataAvailableEventArgs e)
        {
            ...
            int samplesPerChannel = 7;
            float[,,] values = new float[samplesPerChannel ,3, 3]; // Change to add more sensors
            for (int sampleNumber = 0; sampleNumber < samplesPerChannel; sampleNumber = sampleNumber + 1) // This loops captures data from sensor # sampleNumber+1
            {
                // Console.WriteLine("Sensor #" + 15 + " angular velocity X: " + e.GyroscopeSamples[12, 0, sampleNumber] +" "+ sampleNumber); //write sensor values in console
                //Console.WriteLine("Sensor #" + 15 + " angular velocity Y: " + e.GyroscopeSamples[13, 0, sampleNumber] + " " + sampleNumber); //write sensor values in console
                ...
                int sensorNum = sampleNumber + 8;

                        for (int i = 0; i < 3; i = i + 1)
                        {
                            values[sampleNumber, 0, i] = e.GyroscopeSamples[sensorNum, i, sampleNumber];
                            Console.WriteLine("Sensor #" + sensorNum + " angular velocity: " + e.GyroscopeSamples[sensorNum, i, sampleNumber] + " " + i);
                        }

                        for (int i = 0; i < 3; i = i + 1)
                        {
                            values[sampleNumber, 1, i] = e.AccelerometerSamples[sensorNum, i, sampleNumber];
                            Console.WriteLine("Sensor #" + sensorNum + " acceleration: " + e.AccelerometerSamples[sensorNum, i, sampleNumber] + " " + i);
                        }

                        for (int i = 0; i < 3; i = i + 1)
                        {
                            values[sampleNumber, 2, i] = e.MagnetometerSamples[sensorNum, i, sampleNumber];
                            Console.WriteLine("Sensor #" + sensorNum + " magnetic field: " + e.MagnetometerSamples[sensorNum, i, sampleNumber] + " " + i);
                        }
